...
public class QuickDraw {
	...
	/* An integer CORNER, CORNERS, or CENTER (0, 1, or 3) that specifies how to interpret 
	 * the x1, y1, x2, and y2 data within the `create()` function
	 */

	int modeSet;

	/* An integer LEFT, RIGHT, or CENTER (37, 39, or 3) that specifies how to modify 
	 * the x1 and x2 data within the `create()` function after being interpreted by the
	 * `modeSet()` function
	 */

	int alignXSet;

	/* An integer TOP, BOTTOM, or CENTER (101, 102, or 3) that specifies how to modify
	 * the y1 and y2 data within the `create()` function after being interpreted by the
	 * `modeSet()` function
	 */
	...
	public QuickDraw (PApplet p, String s) {  
		...
		// Sets the default drawing alignment interpretation to LEFT and TOP
		alignXSet = PConstants.LEFT;
		alignYSet = PConstants.TOP;
	...
	public void create(float x1, float y1, float x2, float y2, int index, float start, float stop) { 
		...
		for (int k = startAmt; k < stopAmt; k++) {
			...
			/* Determine the difference between the largest x and y values and the edges of
			 * the bounding box used for padding the different sides of the bounding box to
			 * realign the drawing inside.
			 */

			float alignSpaceX = 255 - PApplet.max(xConcat);
			float alignSpaceY = 255 - PApplet.max(yConcat);

			/* Stores the x values and re-maps it to the input scale. The simplified data files
			 * come stored in 255 by 255 coordinate squares. The marker character in xConcat is
			 * excluded from re-mapping so that it remains unmodified and can be properly used
			 * below to identify the end of a line.
			 */

			float x;
			x = xConcat[k];
			if (x != -1) {
				if (alignXSet == PConstants.RIGHT) {
					x += alignSpaceX;
				} else if (alignXSet == PConstants.CENTER) {
					x += (alignSpaceX/2);
				}
				x = PApplet.map(x, 0, 255, -x2/2, x2/2);
			}

			/* Stores the y value and re-maps it to the input scale.The simplified data files
			 * come stored in 255 by 255 coordinate squares.
			 */

			float y;
			y = yConcat[k];

			if (alignYSet == PConstants.BOTTOM) {
				y += alignSpaceY;
			} else if (alignYSet == PConstants.CENTER) {
				y += (alignSpaceY/2);
			}
			y = PApplet.map(y, 0, 255, -y2/2, y2/2);
	 ...
	 * is mode(CENTER), which interprets the first two parameters of create() as
	 * the shape's center point, while the third and fourth parameters are its width and
	 * height.
	 * 
	 * mode(CORNER) interprets the first two parameters of rect() as the upper-left
	 * corner of the shape, while the third and fourth parameters are its width and height.
	 * 
	 * mode(CORNERS) interprets the first two parameters of rect() as the location
	...
	/**
	 * Modifies the alignment of how drawings are created within their bounding boxes.
	 * Controlling the interpretation of the box itself is done using the mode() function.
	 * This function was modeled after Processing's built in textAlign() function.
	 * 
	 * By default drawings have been aligned to their LEFT, TOP corner, scaled
	 * until reaching the first constraining border, RIGHT or BOTTOM. Drawings that are
	 * taller than they are wide take up the full height of the bounding box and cannot
	 * be aligned vertically. Drawings that are wider than they are tall take up the full
	 * width of the bounding box and cannot be aligned horizontally. Therefore, while both
	 * X and Y properties are used, only one will appear active when compared to
	 * the original drawing.
	 *  
	 * The parameter must be written in ALL CAPS because Processing is a case-sensitive
	 * language. The built in variables LEFT, RIGHT, TOP, BOTTOM, and CENTER equate to
	 * the integers 37, 39, and 101, 102, and 3 respectively, which can also be input as
	 * parameters.
	 * 
	 * @param alignX    int: LEFT, RIGHT, or CENTER
	 * @param alignY    int: TOP, BOTTOM, or CENTER
	 */


	public void align(int alignX, int alignY) {
		alignXSet = alignX;
		alignYSet = alignY;
	}


	public void align(int alignX) {
		alignXSet = alignX;
		alignYSet = PConstants.TOP;
	}
